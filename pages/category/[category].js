...
import { mediaBesersUrl } from "../libs/constants";

export async function getServerSideProps({ params }) {
  const response = await fetch(
    `${process.env.API_URL}/category/${params.tags}`
  );
...
const CategoryDetail = ({ cate }) => {
  ...
  return (
    <div className="px-5">
      ...
      {/* detail */}
      <div className="grid lg:grid-cols-3 lg:gap-5 gap-0 article mb-5 relative">
        {/* big article */}
        {data.map((item) => {
          <div className="lg:col-span-2 md:col-span-1 article_img rounded-md mb-5">
            <h1 className="big_article_title">
              ស្អាតម្ល៉េះទេលោក! មួយឈុតនេះ ស្រីនា សមខ្លាំង ឡេីងស្លីមខ្លាំង
            </h1>
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div>
                <article className="space-y-2 mb-3">
                  <div>
                    <a className="font-semibold text-lg dark:text-white hover:text-primary-100 cursor-pointer">
                      ខ្ចិលប្រើឆ័ត្រពេលភ្លៀង!
                    </a>
                    <p className="text-sm">
                      បុរសម្នាក់អាយុ៤២ឆ្នាំ បានចំណាយពេល២ឆ្នាំ ...
                  ...
                  <div className="flex items-center gap-2">
                    <h3 className="text-xs border-2 text-white p-1 bg-primary-100 rounded-lg w-auto">
                      កីទ្បា
                    </h3>
                    <h4 className="text-xs">
                      Written by{" "}
                      <span className="font-bold text-xs text-primary-100">
                        Summer
                      </span>{" "}
                    </h4>
                  </div>
